...
type DegreeObject =
  ...
  | DegreesGroupedByField[];

export async function setCacheDegrees(
  obj: DegreeObject,
...
export async function getCacheDegrees(
  forUpload: boolean = false
): Promise<DegreeObject> {
  if (forUpload)
    return (await redis.get("degressUploadFormat")) as DegreeObject;
