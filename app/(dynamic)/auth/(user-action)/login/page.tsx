    ...
    <div className="flex h-[100vh] w-screen justify-center bg-[#E5F7EB] items-center">
      <div className="md:h-[98vh] h-[80vh] md:w-[85vw]  lg:w-[48vw]   p-2 rounded-lg rounded-bl-[50px] bg-white flex flex-col md:flex-row items-center justify-between ">
        ...
        <div className="h-full   flex justify-center px-4 flex-col   max-w-sm sm:max-w-md  md:w-1/2 lg:w-1/2">
          ...
          <button onClick={() => callbackUrl?.length ? signIn('google', { redirectTo: callbackUrl }) :signIn('google')} className="w-full text-center justify-center px-1 py-2 mb-3 bg-white text-gray-600  border-2 border-slate-200 flex items-center hover:bg-gray-100 transition">
            <Image
          ...
          <p className="mt-4 text-center text-sm text-gray-600 rounded-xl border-2 border-slate-200 py-2">
            ...
            <button className="text-[#2F8E5B] hover:underline" onClick={() =>callbackUrl?.length ? router.push(`/auth/register?callback=${encodeURI(callbackUrl)}`) :router.push("/auth/register")}>
              Sign up
            </button>
          </p>
        </div>
